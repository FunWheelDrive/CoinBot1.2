...
# Logging setup (minimal for Raspberry Pi)
logging.basicConfig(
    level=logging.INFO,
...
LIVE_BALANCE_CHECK_INTERVAL = 30  # Check balance every 30 seconds to save resources
PRICE_UPDATE_INTERVAL = 30  # Update prices every 30 seconds

# Kill Switch State
kill_switch_lock = threading.Lock()
kill_switch_equity_open = {}
kill_switch_breach_start = {}

# VPN Configuration
PREFERRED_VPN_SERVER = "nl-ams"
server_mapping = {
    "nl-ams": "Netherlands",
...
def pretty_now():
    """Return formatted current time in Edmonton timezone."""
    try:
...
def check_vpn_status():
    """Check VPN status using surfshark-vpn command."""
    try:
...
def monitor_live_balance():
    """Monitor live balance for kill switch activation."""
    while True:
        with live_trading_lock:
            state = load_live_trading_state()
...
def update_prices_periodically():
    """Periodically update prices to reduce API calls."""
    global latest_prices, last_price_update, last_price_update_dt
    while True:
        all_symbols = set(coinex_pairs.keys())
        prices = {}
        got_one = False
...
def load_kill_switch_state(bot_id):
    """Load kill switch state for a bot."""
...
def save_kill_switch_state(bot_id, state):
    """Save kill switch state for a bot."""
    kill_switch_file = BOTS[bot_id]["kill_switch_file"]
    try:
        with file_lock:
...
def reset_kill_switch_daily():
    """Reset kill switch daily at 11:59 PM."""
...
def schedule_daily_reset():
    """Schedule daily kill switch reset."""
    while True:
        now = datetime.now(ZoneInfo("America/Edmonton"))
    ...
    try:
        ...
        for symbol in account["positions"]:
            for position in account["positions"][symbol]:
                ...
                position["take_profit_pct"] = float(position.get("take_profit_pct", 3.0))
...
def is_in_buy_window(now_time, buy_hours_str):
    """Check if current time is within buy window."""
    import re
...
def calculate_position_stats(positions, prices):
    """Calculate statistics for open positions."""
    position_stats = []
    for symbol, position_list in positions.items():
...
def liquidate_all_positions(bot_id, account, prices, reason="Kill Switch Triggered"):
    """Liquidate all paper trading positions."""
    modified = False
    timestamp = pretty_now()
...
def dashboard():
    ...
    for bot_id in BOTS:
        account = load_account(bot_id)
        position_stats = calculate_position_stats(account["positions"], latest_prices)
        total_margin = sum(pos['margin_used'] for pos in position_stats)
        total_pl = sum(pos['pnl'] for pos in position_stats)
        available_cash = float(account["balance"])
        equity = available_cash + total_margin + total_pl
        ...
        for d in last_7_days:
            ...
            for log in reversed(logs):
                ...
                rows += (
                    ...
                    f"<td>{format_price(log.get('fee', 0))}</td></tr>"
                )
            if not rows:
                rows = "<tr><td colspan='12'>No trades for this day</td></tr>"
    ...
    for d in live_trade_days:
        ...
        if not rows:
            rows = "<tr><td colspan='9'>No trades for this day</td></tr>"
...
def settings_logout():
    """Log out from settings and live trading."""
...
def webhook():
    """Handle webhook for trading signals."""
    vpn_status = check_vpn_status()
