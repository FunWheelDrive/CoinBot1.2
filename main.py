...
def calculate_position_stats(positions, prices):
    ...
    for symbol, position_list in positions.items():
        ...
        for position in position_list:
            ...
            pnl = (current_price - entry) * volume  # Removed leverage multiplier
...
def webhook():
    try:
        ...
        if action == "buy":
            ...
            profit = (price - avg_entry) * total_volume  # Removed leverage multiplier
            pl_pct = ((price - avg_entry) / avg_entry * 100) if avg_entry > 0 else 0  # Removed leverage multiplier
                ...
                for symbol, positions in account["positions"].items():
                    ...
                    for position in positions:
                        ...
                        if kraken_price > 0 and kraken_price <= stop_loss_price:
                            ...
                            profit = (kraken_price - entry) * volume  # Removed leverage multiplier
                            pl_pct = ((kraken_price - entry) / entry * 100) if entry > 0 else 0  # Removed leverage multiplier
